...
| [`publish-ios-app`](./publish-ios-app/) | Audits, prepares, tests, uploads, submits, and monitors iOS App Store releases. Includes a stateful release ledger, privacy-safe preflight checks, TestFlight gates, subscription guidance, and focused references for native Swift, Capacitor, React Native, and Flutter projects. | ✅ Stable |
| [`verified-research`](./verified-research/) | Multi-agent research pipeline for answers you can defend: decode → discover → gather → adversarial verify → critic → consolidate. Enforces formal+community source balance, cross-checks every key claim against 2+ independent sources, resolves high-impact claims against primary sources, scores every surviving source 0–100 into a persistent ranked Source Ledger, and outputs a readable note where every source is a clickable link with a date + confidence tag. | ✅ Stable |
| [`clip-to-reel`](./clip-to-reel/) | Turns any spoken passage of a long video (talking head, screen recording with a webcam bubble, podcast) into a finished 9:16 explainer reel in one of two proven creator styles you pick first: A (paper split screens, UI-mock scenes, heavy caption chips, snap-zoom cuts) or B (Vox-style animated paper-cut collages generated with Higgsfield, minimal chat/doc UI, small face card). Cuts only on transcription-verified silences, face tracking, SFX, a music bed and a keyword CTA card. Rendered with HyperFrames and scored by an independent judge agent against a reference reel. | ✅ Stable |
